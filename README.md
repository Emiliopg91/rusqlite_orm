...
- a `UserRepository` unit struct implementing `rusqlite_orm::dao::Repository<User>`, with `UserRepository::exists(id)` / `UserRepository::select_by_id(id)` and `UserRepository::select_by_email(email, order_by)` (because of the `#[index("email", (email))]` attribute), plus their `_in` variants,
...
db.run_in_connection(|conn| Ok(user.delete_by_id_in(conn)?))?;
```
